...
int main()
{
	...
	if (!course_load_file("input.txt", &myCourses)) {
		...
		*/
		...
		struct student_list copylist = copyStudentList(&myCourses.first->course.students);
		//descendingBubbleSort(&copylist);
		printStudentList(&copylist);

		struct student_list copylist2 = copyStudentList(&myCourses.first->course.students);
		printStudentList(&copylist2);

		

		mergeStudentList(&copylist, &copylist2);
		printStudentList(&copylist);
